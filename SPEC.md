...
8. **Definitions** from external sources are blockquoted (`>`). When the live page no longer carries a quoted passage, a further anchor on the same line as the source anchor pins the Wayback snapshot it was taken from — `<a href="https://web.archive.org/web/20150221114818/http://en.wiktionary.org/wiki/scope" target="_blank">(archived 2015-02-21)</a>` — and `existence audit --sources` verifies each passage against the live page or any pinned copy; `--fix` writes the pins. Passages quoted from one page in different years carry one pin per year, in line order. The live link stays for readers
9. **Pattern Expression** grounds the abstract in concrete examples. Epistemology subsection headings are non-normative — lint binds only the required `##` sections — so a domain ontology may prefer plainer headings. The recognized subsection vocabulary is `Cultural Definition` | `Sources` and `Pattern Expression` | `Examples` (kernel spelling | plain spelling); `existence lint` warns on any other Epistemology subsection heading without failing. The plain names trade some meaning for readability, so keep the intent: Examples should show the pattern at more than one scale or resolution (not a flat list of instances), and Sources should first quote how the term is already used around you (the domain's cultural definition) before listing references. Nest scale-specific content as `####` subheads under `Pattern Expression` rather than promoting each scale to its own `###` (`#### Human Intelligence`, `#### [Collective](./collective.md) Intelligence`), and put a bare reference anchor in `Cultural Definition` where the sources pass already reads it — lint checks `###` only, so both keep the vocabulary intact
10. **Scope**: definitions are relative to Existence (broadest scope); narrower applications are noted
11. **Typed links** (optional): a link title names the SKOS relation the link asserts — `[information](./information.md "broader")`, `[system](./system.md "narrower")`; an untitled link is `related`. Recognized values: `broader` | `narrower` | `related`. `existence export` emits `skos:broader` / `skos:narrower` / `skos:related` and adds the inverse hierarchy link on the target; `existence lint` warns on any other title and on a target typed both broader and narrower. Type a link only where the hierarchy holds at the term's own scope — a passing mention stays untitled
